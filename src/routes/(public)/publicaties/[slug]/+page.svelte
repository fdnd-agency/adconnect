...
>
	<img
		class="hero-image"
		src={`${DIRECTUS_URL}/assets/${document.hero_image}?format=webp`}
		alt={document.title}
		fetchpriority="high"
	/>
</Hero>
			...
			<div class="file">
				<p>
					Hieronder een preview van het document of bekijk <a
						target="_blank"
						href={`${DIRECTUS_URL}/assets/${document.source_file.id}`}>hier</a
					> het hele document
				</p>
				<iframe
					title={document.title}
					src={`${DIRECTUS_URL}/assets/${document.source_file.id}`}
				></iframe>
			</div>
		{/if}
	</div>
